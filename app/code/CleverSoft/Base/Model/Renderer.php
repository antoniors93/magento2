 ...
 */
namespace CleverSoft\Base\Model;
...
class Renderer implements \Magento\Framework\View\Page\Config\RendererInterface
{
    ...
    public function __construct(
        ...
        $this->string = $string;
        $this->logger = $logger;
        $this->matoHelper = $matoHelper;
        $this->cssTemplate();
    }

    public function cssTemplate() {
        $content = $this->matoHelper->getConfig('cleversoftbase_custom_css/css_swing_editor/custom_css');
        $enable = $this->matoHelper->getConfig('cleversoftbase_custom_css/css_swing_editor/custom_css_enable');
        if (empty($content) || is_null($content) || !$enable) $content = '';
        ...
        //directory list
        $app = $objectManager->get('Magento\Framework\App\Filesystem\DirectoryList')->getPath('app');

        $code_file = $app.'/code/CleverSoft/Base/view/frontend/web/css/custom_css.css';
        //write to original file
        $myfile = fopen($code_file, "w") or die("Unable to open file!");
    ...
    public function renderElementAttributes($elementType)
    {
        ...
        foreach ($this->pageConfig->getElementAttributes($elementType) as $name => $value) {
            if($elementType == 'body' && $name == 'class' && $this->matoHelper->getThemeLayoutCfg('responsive/rtl_language')){
