...
define([
    ...
    'use strict';
    var MTWidgetChooser = Class.create();
    MTWidgetChooser.prototype = {
        ...
        displayEntityChooser: function (container) {
            ...
            if (container && params.url) {
                ...
                params.data = {
                    id: container.id,
                    selected: container.up('div.admin__field-control').down('input[type="text"].entities').value
                };
        ...
        checkCategory: function (event) {
            ...
            var container = event.target.up('div.admin__field-control');
        ...
        checkProduct: function (event) {
            var input = event.memo.element,
                container = event.target.up('div.admin__field-control'),
        ...
        clearEntityValue: function (container) {
            var elm = $(container).up('div.admin__field-control').down('input[type="text"].entities');
            if (elm) elm.value = '';
    ...
    var MTLayoutPreview = Class.create();
    MTLayoutPreview.prototype = {
        MAX_COLUMN: 12,
        COLUMN_WIDTH: 50,
        TITLES: new Hash(),
        ...
        collectFlexGridValues: function (isInit) {
            ...
            if (isInit) {
                ...
                if (!count) {
                    var a = jQuery('#'+this.fieldset +'_block_'+ this.layout);
                    jQuery('#'+this.fieldset +'_block_'+ this.layout).attr('value','');
                    return;
                }
        ...
        bindFlexGridEvent: function () {
            var self = this,
                layout = this.layout,
                container = jQuery('#layout_'+this.fieldset+'_preview_' + layout),
    ...
    window.MTLayoutPreview = MTLayoutPreview;
    return MTWidgetChooser;
});
