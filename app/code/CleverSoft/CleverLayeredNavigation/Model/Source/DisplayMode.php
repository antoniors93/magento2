...
class DisplayMode implements \Magento\Framework\Option\ArrayInterface
{
    ...
    const MODE_SLIDER  = 2;
    const MODE_IMAGE  = 3;
    const MODE_IMG_LABEL  = 4;
    const MODE_TEXTSWATCHES  = 5;
...
//                $options[self::MODE_FROMTOONLY] = __('From-To Only');
            ...
            case self::ATTRUBUTE_INT:
                $options[self::MODE_IMAGE] = __('Images');
                $options[self::MODE_IMG_LABEL] = __('Images & Labels');
                $options[self::MODE_TEXTSWATCHES] = __('Text Swatches');
                break;
            default:
                break;
        }
